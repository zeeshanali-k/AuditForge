...
class MockScanRepository : ScanRepository {
    ...
    override suspend fun triggerScan(sessionId: String, request: TriggerScanRequest): ApiResult<Scan> {
        delay(300)
    ...
    override suspend fun cancelScan(sessionId: String, scanId: String): ApiResult<Scan> {
        delay(200)
        return mutex.withLock {
            val scan = scans[sessionId]?.takeIf { it.id == scanId }
    ...
    override fun observeScanProgress(sessionId: String, scanId: String): Flow<ScanProgressEvent> = flow {
        val steps = listOf(
        ...
        for ((step, percent) in steps) {
            ...
            for (finding in findingsForStep) {
                delay(300)
                findingsSoFar++
                emit(ScanProgressEvent.Finding(finding.id, finding.severity, finding.ruleId, finding.title))
            }
